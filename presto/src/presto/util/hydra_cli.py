...
def default_cfg_path() -> str:
    ...
    presto_cfg_path: str = os.getenv('PRESTO_CFG_PATH', None)
    if presto_cfg_path is not None:
        return presto_cfg_path

    # [2] fallback to presto.cfg
    with importlib.resources.path('presto', 'cfg') as rp:
        return str(rp)
...
def hydra_cli(
    ...
    if argv is not None:
        raise ValueError(
            'I realized that argv is actually ignored in hydra. Do not use!')
    ...
    def decorator(main: Callable[[D], None]):
        ...
        # NOTE(ycho): hydra only works with relative path...
